...
<script>
    function loadIframe() {
        ...
        iframe.setAttribute('src', 'https://247tv.top/m3u8/rriptv.php?c=star1in');
        document.getElementById('playerContainer').appendChild(iframe);

        // Ad-blocking script
        iframe.onload = () => {
            const adBlocker = iframe.contentWindow.document.createElement('style');
            adBlocker.innerHTML = `
                iframe[src*="ads"], 
                div[id*="ad"], 
                [class*="ad"],
            ...
            `;
            iframe.contentWindow.document.head.appendChild(adBlocker);
        };
    }
